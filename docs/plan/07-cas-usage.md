...
> ⚠️ **À retenir** : les chapitres précédents ont posé le modèle. Ici, on l'applique sur des cas réels — pas des toys, des patterns qu'on retrouve dans 90 % des backoffices Symfony.

---

## Slide 7.1 — La règle des deux questions
...
## Slide 7.2 — Quand utiliser Twig Components
...
## Slide 7.3 — Quand utiliser Live Components
...
## Slide 7.4 — Exemple détaillé : recherche live produit
...
## Slide 7.5 — Exemple détaillé : formulaire conditionnel
...
## Slide 7.6 — Exemple détaillé : liste filtrable + pagination
...
## Slide 7.7 — Exemple : quick action (like / bookmark)
...
## Slide 7.8 — Exemple d'école : le compteur
...
## Slide 7.9 — Signaux d'alarme (anti-patterns express)

> Liste à reconnaître **en code review**. La synthèse (chapitre 9) les développe avec solutions et contre-exemples.

| Signal | Diagnostic rapide |
|--------|-------------------|
| Composant sans `LiveProp` writable **ni** `LiveAction` | Ce n'est pas un Live Component — c'est un **Twig Component** |
| `#[LiveProp] public Order $order` (entité complète) | Passer l'**ID**, recharger côté serveur |
| Trois `data-live-action-param` enchaînés sur un clic | Une seule `LiveAction` qui fait les trois étapes |
| Slider, canvas, WYSIWYG, carte interactive en Live | Stimulus pur ou lib dédiée (TipTap, Leaflet…) |

> 🧭 Le **diagnostic détaillé**, les payloads concrets et les mitigations → **chapitre 9** (synthèse).
...
On a le modèle, on a les recettes. Avant de livrer : **comment on teste, comment on profile, comment on sécurise** un Live Component ? Trois sujets qu'on retrouve systématiquement en code review et en prod.
